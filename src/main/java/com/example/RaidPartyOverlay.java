...
public class RaidPartyOverlay extends Overlay {
    ...
    public Dimension render(Graphics2D graphics) {
        if (plugin.getActivePings().isEmpty()) return null;
        ...
        for (RaidPartyPlugin.BossPing p : plugin.getActivePings()) {
            if (now > p.getExpiryTime()) expired.add(p);
        }
        if (!expired.isEmpty()) plugin.getActivePings().removeAll(expired);

        for (RaidPartyPlugin.BossPing ping : plugin.getActivePings()) {
            if (ping.getPoint().getPlane() != client.getPlane()) continue;
            ...
            if (ping.getTargetType() == 1) { // NPC Target
                ...
                if (npc != null) {
                    ...
                    if (floatingText != null) {
                        floatingPoint = npc.getLogicalHeight() > 0 
                            ...
                            : net.runelite.api.Perspective.localToCanvas(client, npc.getLocalLocation(), client.getPlane(), 200);
                    }
                } else {
                ...
                net.runelite.api.Tile tile = client.getScene().getTiles()[client.getPlane()][lp.getSceneX()][lp.getSceneY()];
                ...
                if (tile != null) {
                    ...
                    } else if (tile.getDecorativeObject() != null && tile.getDecorativeObject().getId() == ping.getTargetIndex()) {
                        targetObj = tile.getDecorativeObject();
                    } else if (tile.getGroundObject() != null && tile.getGroundObject().getId() == ping.getTargetIndex()) {
                ...
                if (targetObj != null) {
                    ...
                    if (shapeToDraw == null) shapeToDraw = targetObj.getClickbox();
                    if (shapeToDraw == null) shapeToDraw = Perspective.getCanvasTilePoly(client, lp);
                    ...
                    if (def != null) {
                        ...
                        if (def != null) floatingText = Text.removeTags(def.getName());
                    }
                    
                    int hoverHeight = 150 + (int) (Math.sin(System.currentTimeMillis() / 200.0) * 20); // Bobbing up and down
            ...
            if (shapeToDraw != null) {
                ...
                graphics.setColor(new Color(baseColor.getRed(), baseColor.getGreen(), baseColor.getBlue(), (int)(50 * lifespanPct)));
                graphics.fill(shapeToDraw);
                
                // Emulate a blurred glow by drawing multiple fading thick strokes
                for (int i = 0; i < 5; i++) {
                    float glowAlpha = 1f - (i / 5f);
                    graphics.setColor(new Color(baseColor.getRed(), baseColor.getGreen(), baseColor.getBlue(), (int)(100 * glowAlpha * lifespanPct)));
                    graphics.setStroke(new BasicStroke(2f + (i * 2.5f)));
                    graphics.draw(shapeToDraw);
                }
                
                // Core solid bright line
                graphics.setColor(new Color(baseColor.getRed(), baseColor.getGreen(), baseColor.getBlue(), (int)(255 * lifespanPct)));
                graphics.setStroke(new BasicStroke(2f));
                graphics.draw(shapeToDraw);
            }
            
            // Hovering 3D Icon / Name Renderer
            if (config.drawPingIcons() && floatingPoint != null && lifespanPct > 0.1f && floatingText != null && !floatingText.isEmpty()) {
                ...
                graphics.drawString(floatingText, x + (ping.getTargetType() == 1 ? 1 : 2), y + (ping.getTargetType() == 1 ? 1 : 2));
        ...
        if (plugin.getPartyService() != null && plugin.getPartyService().isInParty()) {
            for (net.runelite.api.Player p : client.getPlayers()) {
                if (p == null || p.getName() == null || p == client.getLocalPlayer()) continue;
                
                String pName = p.getName();
                RaidPartyPlayerSync syncData = null;
                for (RaidPartyPlayerSync s : plugin.getPartyData().values()) {
                    if (pName.compareTo("\u00A0") != 0 && pName.replace('\u00A0', ' ').equals(s.getUsername().replace('\u00A0', ' '))) {
                        syncData = s;
                        break;
                    }
                }
                ...
                if (syncData != null && syncData.getMaxHp() > 0) {
                    ...
                    if (hpPct <= 0.1f && config.enableCriticalHpGlow()) {
                        glowColor = config.criticalHpColor();
                    } else if (hpPct <= 0.3f && config.enableLowHpGlow()) {
                    ...
                    if (glowColor != null) {
                        ...
                        if (hull != null) {
                            ...
                            for (int i = 0; i < 4; i++) {
                                ...
                                graphics.setColor(new Color(glowColor.getRed(), glowColor.getGreen(), glowColor.getBlue(), (int)(120 * glowAlpha * pulse)));
                                graphics.setStroke(new BasicStroke(2f + (i * 3f)));
                                graphics.draw(hull);
                            }
                            
                            // Core solid bright line
                            graphics.setColor(new Color(glowColor.getRed(), glowColor.getGreen(), glowColor.getBlue(), (int)(255 * pulse)));
